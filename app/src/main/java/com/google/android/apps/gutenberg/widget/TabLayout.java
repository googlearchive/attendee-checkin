...
public class TabLayout extends HorizontalScrollView {
    ...
    public void setEnabled(boolean enabled) {
        ...
        for (int i = 0, count = mTabStrip.getChildCount(); i < count; i++) {
            ...
            if (child instanceof TabView) {
                child.setOnClickListener(enabled ? mTabClickListener : null);
                Tab tab = ((TabView) child).getTab();
                if (tab.mCustomView != null) {
                    tab.mCustomView.setClickable(!enabled);
                }
            }
        }
    }
    ...
    /**
     * Set the {@link OnTabSelectedListener} that will handle switching to and from tabs.
     *
    ...
    private static boolean isViewLaidOut(View view) {
